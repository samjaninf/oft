		...
		if ( is_404() ) {
			...
			if ( ! empty( $taxonomies ) ) {
				...
				if ( ! empty( $terms ) ) {
					...
					// GEWIJZIGD: Neem niet gewoon het eerste element uit de array, maar zoek 'product_cat' op (indien beschikbaar)
					$key = 0;
					foreach ( $lowest_term as $index => $available_term ) {
						if ( $available_term->taxonomy === 'product_cat' ) {
							$key = $index;
						}
					}
					
					$term = $lowest_term[$key];
